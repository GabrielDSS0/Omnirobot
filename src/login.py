			...
			if len(splitMsg) > 1:
				if splitMsg[1] == "challstr":
					...
					sending.call_command(
						self.websocket.send(
							f"|/trn {config.username},0,{assertion}"
						)
					)
					sending.call_command(
						self.websocket.send(f"|/avatar {config.avatar}")
					)

					for room in config.rooms:
						sending.call_command(
							self.websocket.send(f"|/join {room}")
						)
					...
					sending.call_command(
						self.websocket.send(
							"|/status Para saber os comandos, digite @commands em minha PM."
						)
					)
	...
	async def afterLogin(self):
		...
		if cmd_or_invite == "COMMAND":
			...
			if command in cmd_list.commands_misc:
				await (
					misc_redirect.RedirectingFunction().redirect_to_function()
				)
				...
				await (
					admin_redirect.RedirectingFunction().redirect_to_function()
				)
