...
class GameCommands():
    ...
    def startdp(self):
        ...
        self.started = True
        sending.respondRoom("O jogo de Dungeons & Pokémon foi iniciado! Para definir os jogadores, o host do jogo terá que digitar @defplayers [jogadores] aqui no chat, e eu mesmo irei sortear os jogadores por equipe.", self.room)
        sending.respondRoom("Post oficial do fórum de Dungeons & Pokémon, caso tenha dúvidas em relação ao jogo: https://pspt.boards.net/thread/701/projeto-dungeons-pokemon", self.room)
    ...
    def defplayers(self):
        ...
        if not self.players:
            self.players.clear()
            return sending.respond(self.msgType, f"Uso do comando: {config.prefix}defplayers [jogadores separados por vírgula]", self.senderID, self.room)
        if (len(players) % 2) != 0:
            self.players.clear()
            return sending.respond(self.msgType, f"O número de jogadores deve ser par.", self.senderID, self.room)
        ...
        sending.respondRoom(f"Equipe 1: {', '.join(self.team1)} \n Equipe 2: {', '.join(self.team2)}", self.room)
        self.priorityTeam = random.choice([self.team1, self.team2])
        team = "Equipe 1" if self.priorityTeam == self.team1 else "Equipe 2"
        sending.respondRoom(f"A equipe que terá prioridade de habilidades no primeiro round será a: {team}", self.room)
        
        self.threading_timer(2, sending.respondRoom, ["Equipes definidas! Peço agora ao host para que defina as classes dos respectivos jogadores com o comando @defclass em minha PM.", self.room])
    ...
    def defclass(self):
        ...
        if player not in self.players:
            return sending.respondPM(self.senderID, "O usuário indicado não está na sala indicada.")
        if not (player_class in classes.classes_dict):
            return sending.respondPM(self.senderID, "A classe indicada não existe.")
        ...
        if len(list(self.playersClasses)) == len(self.players):
            sending.respondPM(self.senderID, "Todas as classes foram atribuídas!")
            code = "Classes:\n"
            ...
            self.threading_timer(3, sending.respondPM, [self.senderID, "Estas são as classes que deseja? Caso sim, digite @confirmclass para confirmar."])
    ...
    def confirmclass(self):
        ...
        if len(list(self.playersClasses)) == len(self.players):
            sending.respondPM(self.senderID, "Todas as classes estão definidas!")
            for player in self.playersClasses:
                player_class = self.playersClasses[player]
                special_ability = player_class.default_abilities[-1]
                special_ability_cooldown = abilities.abilities_dict[special_ability].cooldown
    ...
    def act(self):
        ...
        if not (player in self.playersClasses):
            return sending.respondPM(self.senderID, f"{player} não está na partida.")
        
        player_class = self.playersClasses[player]
        act_name = self.commandParams[1].strip()
        if not (act_name in abilities.abilities_dict):
            return sending.respondPM(self.senderID, f"{act_name} não é uma habilidade válida.")
        ...
        if targets:
            sending.respondPM(self.senderID, f"{player} utilizará a habilidade {act_name}. Alvos: {', '.join(targets)}")
        else:
            sending.respondPM(self.senderID, f"{player} utilizará a habilidade {act_name}.")
        self.abilities_order[player] = act
    
    def cancelact(self):
        if not self.abilities_order:
            return sending.respondPM(self.senderID, "Não há ações a serem canceladas.")
    ...
    def actsconfirm(self):
        ...
        postRoundInstance: endround.PostRound = endround.PostRound(self.idGame, self.room, self.playersClasses, self.team1_classes, self.team2_classes, self.playersDead, self.team1_dead, self.team2_dead)
    ...
    def verify_spirit_trapper(self):
        for player in self.playersClasses:
            ...
            if player_class.name == "Spirit" and self.round == 1:
                sending.respondPM(self.senderID, f"É necessário anexar um possuído para o Spirit {player}. Digite @spirit [jogador], [possúido]")
            if player_class.name == "Trapper":
                sending.respondPM(self.senderID, f"É necessário plantar uma armadilha de Trapper em um aliado de {player}. Digite @trapper [jogador Trapper], [jogador-armadilha]")

    def spirit(self):
        player_spirit = self.commandParams[0].strip()
        if not (player_spirit in self.playersClasses):
            return sending.respondPM(self.senderID, f"{player_spirit} não está entre os jogadores da partida.")
        player_possessed = self.commandParams[-1].strip()
        if not (player_spirit in self.playersClasses):
            return sending.respondPM(self.senderID, f"{player_possessed} não está entre os jogadores da partida.")
        ...
        sending.respondPM(self.senderID, f"{player_possessed} foi possuído por {player_spirit}!")
    ...
    async def makehost(self):
        ...
        for room in rooms:
            ...
            if substringRoom == room:
                sending.respond(self.msgType, f"Novo host: {new_host}", self.senderID, self.groupchat_name_complete)
                instance = self.dpGames[self.host][self.groupchat_name_complete]
                vars.Varlist.dpGames[new_host_id] = {self.groupchat_name_complete: instance}

                if new_host_id in vars.Varlist.hosts_groupchats:
                    groupchats: list = vars.Varlist.hosts_groupchats[new_host_id]
                    groupchats.append(room)
                    vars.Varlist.hosts_groupchats[new_host_id] = groupchats
                else:
                    vars.Varlist.hosts_groupchats[new_host_id] = [self.groupchat_name_complete]
                return True
        return
    ...
    def finishdp(self):
        ...
        sending.respondRoom("O jogo de Dungeons & Pokémon foi encerrado.", self.room)
        self.delete_game()
