...
class GameCommands():
    ...
    def makequestion(self):
        ...
        sending.respondPM(self.senderID, f"Questão feita! Agora, para adicionar alternativas, digite {config.prefix}add (alternativa).")
    ...
    def cancelquestion(self):
        if not self.question:
            return sending.respondPM(self.senderID, "Não há uma questão ativa.")
        self.killQuestion()
    ...
    def addalternative(self):
        ...
        sending.respondPM(self.senderID, f"Alternativa feita! Se quiser colocar alguma alternativa como a correta, digite {config.prefix}danswer (alternativa).")
    ...
    def defineanswer(self):
        ...
        if alternative in self.alternatives:
            ...
            sending.respondPM(self.senderID, f"A alternativa {alternative} foi configurada como a correta.")
        else:
            sending.respondPM(self.senderID, f"A alternativa inserida não é uma das alternativas da questão.")
    ...
    def respondquestion(self):
        ...
        if self.currentQuestion:
            if self.senderID in self.usersAnswered:
                return sending.respondPM(self.senderID, "Você já enviou uma resposta nesta questão.")
            else:                
    ...
    def timeLimit(self):
        ...
        sending.respondRoom(f"/wall ACABOU O TEMPO!", self.room)
        self.postQuestion()
    ...
    def postQuestion(self):
        ...
        threads.append(threading.Timer(pre_answer_revelation, sending.respondRoom, args=["E a resposta era...", self.room]))
        threads.append(threading.Timer(answer_revelation, sending.respondRoom, args=[f"/wall {self.answer}!", self.room]))
        threads.append(threading.Timer(score, sending.respondRoom, args=[f"Pontuadores: {', '.join(self.usersPointers)}", self.room]))
    ...
    def finishQuestion(self):
        self.killQuestion()
        sending.respondPM(self.host, "Acabou o prazo para formalizar a questão.")
