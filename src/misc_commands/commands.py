...
class Misc_Commands:
	...
	async def commands(self):
		response_user_rooms = await self.query(
			"userdetails", f"{self.senderID}"
		)

		user_rooms = set((json.loads(response_user_rooms[3])["rooms"].keys()))

		response_bot_rooms = await self.query(
			"userdetails", f"{config.username}"
		)
		...
		for room in bot_rooms:
			...
			for user_room in user_rooms:
				...
				if (user_room == room_no_symbol) and (
					room_symbol in config.send_html
				):
					with open(
						config.commands_file, "r", encoding="utf-8"
					) as html_file:
						html_content = html_file.read()
						converted = BeautifulSoup(html_content, "html.parser")
						html_done = html.unescape(
							str(converted).replace("\n", "")
						)

						sending.call_command(
							self.websocket.send(
								f"{room_no_symbol}|/pmuhtml {self.senderID},htmlcommands,{html_done}"
							)
						)
		...
		sending.respondPM(
			self.sender,
			"Você não está em uma sala em que o bot tem os cargos *, # ou ~.",
		)

	def addpoints(self, newPoints=1, fromRespond=False):
		if self.msgType:
			points_receiver = (
				self.commandParams[0]
				if self.msgType == "room"
				else self.commandParams[1]
			)
			...
			try:
				...
				return sending.respond(
					self.msgType,
					"A pontuação digitada não é válida.",
					self.senderID,
					self.room,
				)
		else:
		...
		if idUser_fetch:
			...
			user_room_lb = self.sql_commands.select_userpoints_leaderboard(
				user, room
			)
			...
			if user_room_lb:
				...
				if points <= 0:
					...
					self.sql_commands.update_userpoints_leaderboard(
						points, user, room
					)
			else:
				if newPoints <= 0:
					return sending.respond(
						self.msgType,
						"Só se pode adicionar valores de pontos maiores que 0.",
						self.senderID,
						self.room,
					)
		...
		if not fromRespond:
			sending.respond(
				self.msgType, "Pontos adicionados!", self.senderID, self.room
			)
	...
	def removepoints(self, remPoints=1):
		...
		if idUser_fetch:
			...
			if user_room_lb:
				...
				return sending.respond(
					self.msgType,
					"O usuário citado não tem pontos nesta sala.",
					self.senderID,
					self.room,
				)
		else:
		...
		sending.respond(
			self.msgType, "Pontos removidos!", self.senderID, self.room
		)
	...
	def clearpoints(self):
		...
		sending.respond(
			self.msgType, "Pontos da sala limpos!", self.senderID, self.room
		)

	def leaderboard(self, inQuestion=False):
		if not (inQuestion):
			if (
				self.commandParams[0] not in config.rooms
				and self.msgType != "room"
			):
				return sending.respond(
					self.msgType,
					"Não estou na sala descrita.",
					self.senderID,
					self.room,
				)
		...
		lbSequenceSorted = dict(
			sorted(lb.items(), key=lambda item: item[1], reverse=True)
		)
	...
	def timer(self):
		...
		try:
			...
			return sending.respond(
				self.msgType,
				"O tempo inserido não é um número.",
				self.senderID,
				self.room,
			)
		...
		if not (unit == "min" or unit == "sec"):
			return sending.respond(
				self.msgType,
				"A unidade de medida deve ser ou min ou sec.",
				self.senderID,
				self.room,
			)
		...
		sending.respond(
			self.msgType, "Timer iniciado!", self.senderID, self.room
		)

		timer_thread = threading.Timer(
			time,
			sending.respond,
			args=[self.msgType, "Tempo encerrado!", self.senderID, self.room],
		)
