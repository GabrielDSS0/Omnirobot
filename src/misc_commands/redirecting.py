...
class RedirectingFunction:
	...
	async def redirect_to_function(self):
		...
		elif self.msgType != "room" and len(self.commandParams) < len(
			command_params_default
		):
		...
		if command_permission == "adm" or (
			command_permission == "general" and self.msgType == "room"
		):
